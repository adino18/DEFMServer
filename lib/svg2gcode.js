...
module.exports = {
	svg2gcode : function(svg, settings) { //lấy setting và các thuộc tinh của hình
		...
		settings.scale = 	settings.scale || 100;	// scale 
		settings.feedRate = settings.feedRate || 500;	//engraving speed
		...
		var
			scale=function(val) {             ///scale hình dựa trên setting
	    		// return val * (scale/100)*unitConst.px;
				return val * (settings.scale/100)*unitConst.px;
	  		},
